...
/**
 * In this update policy, state is located on the server and clients must request state initially.
 */
public class SharedEMMUpdatePolicy implements UpdatePolicy<Integer, String, String> {

    private @Nonnull final TokenGenerator generator;

    public SharedEMMUpdatePolicy(final TokenGenerator generator) {
    ...
    public void update(ServerInterface<Integer, String, String> server,
        ...
        for (String plaintextLabel : plaintextRecords.keySet()) {
            ...
            Integer counter = claimCounterRange(server, context, plaintextLabel,
    ...
    private Integer claimCounterRange(ServerInterface<Integer, String, String> server,
                                      ClientContext context,
                                      String label, int count) {
        while (true) {
            // TODO: Actually encrypt these values.
            Integer startOfRange = server.getLabelState(context, label);
            Integer endOfRange = startOfRange + count;
            if (server.reserveLabelRange(context, label, startOfRange, endOfRange)) {
                return startOfRange;
            }
        }
    }
